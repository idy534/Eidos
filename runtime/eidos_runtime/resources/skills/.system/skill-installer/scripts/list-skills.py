...
def _request(url: str) -> bytes:
    return github_request(url, "eidos-skill-list")


def _eidos_home() -> str:
    return os.environ.get("EIDOS_DATA_DIR", os.path.expanduser("~/.eidos"))


def _installed_skills() -> set[str]:
    root = os.path.join(_eidos_home(), "skills")
    ...
    if os.path.isdir(system):
        entries.update(
            name for name in os.listdir(system)
...
def _list_skills(repo: str, path: str, ref: str) -> list[str]:
    ...
    try:
        ...
        raise ListError("GitHub returned an invalid skills listing.") from exc
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        return 0
    except (ListError, ValueError, urllib.error.URLError) as exc:
