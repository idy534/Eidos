...
from eidos_runtime.persistence.errors import ConditionalUpdateFailed, PersistenceCorruptionError
...
class LongTaskRepository(Repository):
    def initialize(self, *, run_id: str, **values: object) -> LongTaskProgress:
        ...
        progress = LongTaskProgress(
            ...
            permission_snapshot_hash=_optional_text(values.get("permission_snapshot_hash")),
    ...
    def mark_paused(self, run_id: str, safe_point: SafePoint) -> LongTaskProgress:
        current = self._require(run_id)
        if current.status is not LongTaskStatus.PAUSE_REQUESTED or safe_point not in _PAUSABLE_SAFE_POINTS:
            raise ConditionalUpdateFailed("pause safe point is unavailable")
        return self._update(
            current, status=LongTaskStatus.PAUSED, safe_point=safe_point,
            paused_at=_now_ms(),
        )
    ...
    def record_verification(
        ...
        current = self._require(run_id)
        if current.status is not LongTaskStatus.RESUME_REQUESTED or verification.run_id != run_id:
        ...
        if verification.status is not VerificationStatus.VERIFIED:
            raise ConditionalUpdateFailed("resume verification is blocked")
    ...
    def mark_resumed(self, run_id: str) -> LongTaskProgress:
        ...
        if (
            ...
            or current.last_verification.status is not VerificationStatus.VERIFIED
        ):
            raise ConditionalUpdateFailed("resume requires verified state")
        return self._update(
            current, status=LongTaskStatus.RUNNING,
            safe_point=SafePoint.BEFORE_MODEL, resumed_at=_now_ms(),
        )
    ...
    def request_cancel(self, run_id: str) -> LongTaskProgress:
        ...
        if current.status in _TERMINAL:
            return current
        return self._update(
            current, status=LongTaskStatus.CANCEL_REQUESTED,
    ...
    def mark_canceled(
        ...
        return self._update(
            ...
            side_effects_may_exist=(current.side_effects_may_exist or side_effects_may_exist),
            reconciliation_required=(current.reconciliation_required or side_effects_may_exist),
        )
    ...
    def _insert(self, progress: LongTaskProgress) -> None:
        ...
        with self.lock, self._connection() as connection:
            connection.execute(
                ...
                """,
                (
                    progress.run_id, CONTROL_SCOPE, request_hash,
                    progress.model_dump_json(), progress.updated_at,
                ),
            )
    ...
    def _update(self, current: LongTaskProgress, **changes: object) -> LongTaskProgress:
        ...
        with self.lock, self._connection() as connection:
            changed = connection.execute(
                ...
                """,
                (
                    updated.model_dump_json(), current.run_id, CONTROL_SCOPE,
    ...
    def verify(
        *,
        ...
        return ResumeVerification(
            run_id=run_id,
            status=VerificationStatus.BLOCKED if reasons else VerificationStatus.VERIFIED,
...
class RestartVerifier:
    @staticmethod
    def verify(progress: LongTaskProgress, verification: ResumeVerification) -> ResumeVerification:
        if progress.side_effects_may_exist and "reconciliation_required" not in verification.reasons:
