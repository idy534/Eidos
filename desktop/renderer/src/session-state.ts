...
export function taskStatusPresentation(
  ...
  if (activeRunStatus === "waiting_agents") return { label: "等待子任务", tone: "progress", spinning: false };
...
export function taskStatusFromRun(
  ...
  if (["queued", "running", "waiting_input", "waiting_agents", "waiting_approval", "finalizing"].includes(run.status)) {
    return "in_progress";
  }
...
const RUNTIME_ERROR_MESSAGES: Record<string, string> = {
  ...
  WORKSPACE_FILE_CHANGED: "文件已更新，请刷新预览。",
  WORKSPACE_PREVIEW_UNSUPPORTED: "当前文件类型不支持内嵌预览。",
  GIT_DIFF_CHANGED: "文件差异已变化，请刷新后重新选择修改块。",
  GIT_HUNK_UNAVAILABLE: "此修改不能按块操作，请使用整文件操作。",
  GIT_PATCH_UNAVAILABLE: "无法读取文本修改块，请使用整文件审阅。",
  ...
  WORKTREE_RESTORE_REQUIRED: "本地工作树需要恢复。请先恢复工作树，再更改工作环境。",
  HANDOFF_NOT_SUPPORTED: "当前任务不支持更改工作环境。",
  HANDOFF_SOURCE_CHANGED: "当前工作环境已经发生变化。请刷新后重试。",
  HANDOFF_TARGET_CHANGED: "目标工作环境已经发生变化。请刷新后重试。",
  HANDOFF_LOCAL_CONFLICT: "当前工作树的修改无法安全同步到本地。请先处理本地冲突。",
  HANDOFF_GIT_CONFLICT: "当前版本状态存在冲突，无法安全更改工作环境。",
