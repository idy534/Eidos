...
- `GitBackend` 把 Git mechanics 与 Eidos Worktree lifecycle 分开。Eidos 不实现 Git working-tree、Index、commit、fast-forward 或 push semantics。Dulwich 负责 discovery、HEAD、refs、branch metadata 和 revision resolution。唯一的 `GitCli` authority 负责 status、diff、stage、unstage、commit、fetch、fast-forward merge、push、patch mechanics、Worktree Add 和 destructive clean；它不作为 Dulwich failure retry path。Commit 读取 repository-local identity，或只从用户 global config 读取 `user.name` 和 `user.email`。Hooks 始终禁用。Credentials 只在 REMOTE profile 开放。Observation command 禁用 executable filters。Stage 允许 native Git 执行 clean/process filter。
