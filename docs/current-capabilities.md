...
- Runtime 提供 typed `session/gitStage`、`session/gitUnstage` 和 `session/gitCommit`。Stage 使用 path-scoped `git add --all -- <paths>`。Unstage 在正常 HEAD 上使用 `git restore --staged -- <paths>`，并处理 unborn HEAD。Commit 只提交 staged changes。成功结果会重新观察并返回 `head`、`branch` 和 structured status。Commit 还返回等于当前 HEAD 的 `commit`。Active Run 会返回 `GIT_WORKFLOW_BUSY`。Detached managed Worktree 会返回 `GIT_BRANCH_REQUIRED`。
