...
- Managed Checkpoint 会引用独立的 durable Worktree Snapshot。Create 固定当时的 HEAD、staged patch、unstaged patch 和 untracked content。Fork 会从该 HEAD 创建新的 detached managed Worktree，并恢复完全相同的 Index 和 Working Tree 状态。Rewind 会在原 managed Worktree 上执行 native `git reset --hard`、`git clean -fdx` 和 Git patch apply，然后再次验证 Snapshot fingerprint。Checkpoint 引用的 artifact 和 hidden ref 不会被普通 retention snapshot replacement 删除。
