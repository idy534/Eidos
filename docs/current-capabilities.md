...
- `search_text` 使用随 Runtime 管理、manifest 校验和 SHA256 校验的 macOS arm64 Ripgrep 资源。Runtime 分开管理搜索进程寿命和单次等待窗口。`search_text` 默认等待 10 秒，可设置 250 毫秒至 30 秒；等待窗口结束会返回 `search_running` 和 Run 内 `sessionId`，不会结束 Ripgrep。模型可以用 `search_text_wait` 继续等待，默认 30 秒，范围 250 毫秒至 60 秒。Run 结束或取消时，Runtime 会取消仍在运行的搜索。单次搜索进程的安全上限是 600 秒。
...
- Local Git Session 支持在当前 Project workspace 的 local branches 之间切换，也支持从当前 HEAD 创建并切换到新 branch。Desktop Composer 和 Changes 视图都提供 Local branch selector；创建分支复用现有 Create Branch 对话框。Runtime 会拒绝 dirty workspace、active Run、进行中的 Git operation 和非 Local Session。多个 Local Session 共享同一个 workspace，因此 branch mutation 和 Run admission 的短暂 preflight 使用同一条 workspace 锁；这条锁不限制已启动 Run 的 Shell 或普通副作用并行。
- Runtime 可以返回 HEAD diff 和 baseline diff。Managed Worktree 的 baseline 使用创建时 immutable `base_commit`。Local Session 的 baseline 默认使用可解析的 upstream remote-tracking ref；没有可解析 upstream 时回退 HEAD。调用者也可以显式传入 `compareRef`。无效 ref 返回 `GIT_COMPARE_REF_INVALID`。Diff 包含 tracked 和 untracked files，并支持单个 workspace-relative path。响应使用 native Git `--numstat` 返回总 additions、总 deletions 和每个文件的 additions、deletions。二进制文件会设置对应文件的 `statsIncomplete`。这些统计不受 unified patch 截断影响。Diff 不修改真实 Git Index 或 object store；untracked 批量经系统 tmp 内一次性 `GIT_INDEX_FILE` 完成，失败回退有界逐文件路径。未指定 `path` 且 untracked 超过 200 个时，`changed_files/file_stats` 取前 200 并置 `truncated/statsIncomplete`，单文件 `path` 请求不受该上限影响。
- Runtime 提供 typed `session/gitStage` 和 `session/gitCommit`。File path 通过 `GIT_LITERAL_PATHSPECS=1` 交给 Git。Stage 使用 path-scoped `git add --all -- <paths>`，并保留 native Git 的 repository/user clean filter 语义。Commit 只提交 staged changes。成功结果会重新观察并返回 `head`、`branch` 和 structured status。Commit 还返回等于当前 HEAD 的 `commit`。Active Run 会返回 `GIT_WORKFLOW_BUSY`。Detached managed Worktree 会返回 `GIT_BRANCH_REQUIRED`。
...
- 只读 `project/gitContext`、`session/gitStatus`、`session/gitDiff` 同样经 `DeferredMethodResult` 与 managed task  offload，不再占用 JSON-RPC 输入循环；排队中的 `session/read`、`model/presets` 等便宜调用不再被慢 Git 队头阻塞。Draining/重配时调度失败会同步回退，保持读可用。
...
- Linked managed Worktree 的 `git_dir` 和 `git_common_dir` 可以在默认 Seatbelt 中只读访问。获批的 `run_shell.gitWriteAccess` 可以为本次命令写入这两个已核验 root。原始 repository working tree 不在该 Workspace 的访问范围内。
...
- `workspace_dependencies` 优先返回当前已验证 Bundle 的 executables、Python import roots、Python 包版本和 binding IDs，source 为 `eidos_runtime_bundle`。没有 Bundle 数据时，工具使用经过 owner、类型、可执行位和 SHA-256 校验的源码 Runtime catalog，并只报告实际存在于受信任 roots 中的受支持包。模型投影只保留 `code` 和 `data`，canonical result 仍保留 Runtime 状态字段。binding 投影异常会显式返回 `runtimeDependencyError`。模型需要把选定的 `dependencyBindingId` 传给 `run_shell`，再使用绑定的 `RUNTIME_PYTHON` 或 `RUNTIME_NODE`；默认 Shell 环境不因此自动切换。
...
- Skill 脚本复用已有 `run_shell`。受信任的 Catalog Skill 下的相对 `scripts/` 调用可以产生 implicit activation，并把 active Skill root 放入权限快照。Workspace root 是读写；active Skill root 默认只读并允许 executable mapping。普通 Skill 的获批写权限只覆盖审批展示的具体文件或目录，系统 Skill 不接受写入授权。插件脚本的系统依赖不进入 Skill 目录。
...
- `skill_read` 和 `skill_read_resource` 接受 UTF-8 字节 `offset` 与 `maxBytes`，默认及上限为 12,000 bytes。结果返回 `startByte`、`endByte`、`totalBytes` 和可选 `nextOffset`；模型按 nextOffset 续读。工具保留完整资源的 contentHash，并拒绝字符中间的 offset。分页还为 JSON 转义预留空间，避免分页正文再次被通用投影截断。
...
- RunResources 会收集 active Skill 的 MCP dependency，并与本 Run extension snapshot 中 available 的 Plugin 或手动 MCP server 比较，生成 installed、missing 或 unsupported 诊断。未满足项会进入低权限 user context warning。Runtime 不会静默安装、启用或启动 MCP server。Plugin 和手动 MCP 都使用独立的 server consent、官方 Python MCP SDK stdio client、Tool discovery、Tool call、Approval 和 Sandbox 链路。
...
- MCP 当前支持 stdio Tools。Settings 可以创建、编辑和卸载手动 MCP，并配置名称、启动命令、参数、环境变量、环境变量透传名、工作目录和 `connector`/`workspace_read` permission profile。保存或编辑后的 Server 默认未授权，用户需要通过现有审阅入口重新启用。环境变量值不会出现在 MCP 列表、事件或日志中，编辑时留空会保留已保存的环境变量值。Server consent、Tool discovery、Tool call、timeout、结果 schema 和 Tool List Changed bookkeeping 已接入。
...
- 本轮结果卡只在 Run 进入终态且当前回答不再生成后显示，位置在最终答复之后。运行中、等待审批和收尾阶段不显示；失败或取消且没有最终答复时，系统仍展示已经产生的结果。文本修改卡从 Session 快照中的 Run、Item 和持久化 ToolCall 投影，支持单文件紧凑卡、多文件折叠列表、删除记录、失败状态和对应 Run/文件的 Review 入口。卡片不把整个仓库当前差异当成本轮修改。
- 文本修改按本轮路径归组。重复修改且各次补丁完整时，卡片显示累计增删；该统计包含重复编辑，不代表最终净差异。缺少补丁时，系统显示“行数未完整统计”。Shell 只有路径观察而没有可靠 Diff 时不会生成虚假的行数。
- 内置工具 `declare_outputs` 始终随 Run 注册，系统提示要求模型仅对用户期望接收的独立终态交付文件在终版就绪后、最终答复前调用，不依赖系统或第三方 Skill。文件被创建或修改不等于交付物；项目内普通源码、文档、配置等修改仍是工作区变更，除非用户明确要求作为独立文件接收；依赖、缓存、构建产物、源码引用、QA 中间输出默认不声明；拿不准时默认不声明，交付物修订后需再次声明。每次接收 1–20 个 `outputs`，每项包含 `path` 和可选 `title`。路径允许 Workspace 相对路径或当前 Run Workspace 内的绝对路径。工具只核验普通文件的路径、身份、大小和版本，不创建、不修改、不校验文件内容，也不证明作者、内容质量或测试通过；操作是原子的，任一文件失败则整批不声明，重复声明更新同一文件的声明。
...
- 产物卡的专属分类覆盖文档（DOC、DOCX）、PDF、演示文稿（PPT、PPTX）、表格（CSV、TSV、XLS、XLSX、XLSM）、图片（PNG、JPG、JPEG、GIF、WEBP、SVG）和网页（HTML、HTM），后缀匹配不区分大小写；其他格式仍使用通用文件卡。卡片显示分类和实际格式，演示文稿及表格复用文件树的对应图标。DOC/DOCX、PPT/PPTX、XLS/XLSX/XLSM 提供系统应用打开与在 Finder 中显示，CSV/TSV 提供文本预览、系统应用打开与在 Finder 中显示。CSV/TSV 有修改证据时仍可进入文本审查，分类不会将它们当作二进制而丢弃。
...
- Review 提供“最近一轮”范围，严格按 `runId` 过滤持久化 ToolCall 文件补丁，空轮保持空，不回退到全量；“整个任务”展示本 Session 全部 Run 的补丁历史。长任务可以分页加载更早记录。文件工具在 Prepare 后提交 `item.updated` Event，并通过 Outbox 推送 `item/updated`。界面不会把准备中的补丁标记为已写入。
- 最近一轮反馈包含 Run、ToolCall、文件、行位置和已有 Base SHA。
...
- 文本卡的标题、审核按钮和文件行可以打开独立文本审查面板，支持 Git、非 Git 和 Projectless 会话。面板只提供“最近一轮修改”和“整个任务修改”；前者绑定被点击卡片的 Run，后者展示当前 Session 的补丁历史。面板没有 Git 操作，也不在工作区工具列表或环境信息中提供入口。原有 Git 提交入口保留。本次修改未运行测试或 Desktop 验收。

- 环境信息始终属于左侧 Session 标题栏。右侧工作区独立顶格显示，打开后工作区开关位于 Dock 标题栏；关闭时开关回到 Session 标题栏。工作区全宽展开时覆盖左侧标题栏，环境信息不迁入 Dock。按钮统一尺寸、圆角和悬停色，不再使用独立的右上角绝对定位。本次修订未运行测试或 Desktop 视觉验收。

- Session 与 Dock 标题栏统一为 56px 高度。左侧导航通过列宽和位移折叠；右侧工作区通过宽度和位移推入推出，全宽切换沿用相同过渡。右侧关闭后在 280ms 过渡结束时卸载内容；快速重新打开会取消卸载。拖动分隔条时关闭过渡，减少动态效果设置下直接切换。本次动画尚未完成 Desktop 视觉验收。
