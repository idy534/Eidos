...
- MCP 当前支持 stdio Tools。Settings 可以创建手动 MCP，并配置名称、启动命令、参数、环境变量、环境变量透传名、工作目录和 `connector`/`workspace_read` permission profile。保存后的 Server 默认未授权，用户需要通过现有审阅入口启用。环境变量值不会出现在 MCP 列表、事件或日志中。Server consent、Tool discovery、Tool call、timeout、结果 schema 和 Tool List Changed bookkeeping 已接入。
