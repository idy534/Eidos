...
- `agents/eidos.yaml` 是可选 metadata。Runtime 使用统一 YAML parser 读取 interface、asset、dependency 和 policy metadata。无效可选 metadata 会 warning 并忽略。Skill 不管理 pip、npm、系统命令或其他运行时依赖。
...
- Skill metadata 的 MCP/tool dependency 只允许作为检测和 warning 输入，不触发依赖安装或启动。当前 loader 只 boundedly 解析 metadata，解析失败会 warning 并忽略。Plugin MCP 仍使用独立的 server consent、官方 Python MCP SDK stdio client、Tool discovery、Tool call、Approval 和 Sandbox 链路。当前分支没有把 Skill metadata dependency 自动接入 MCP 生命周期。
- 当前 checkout 的 `SkillCatalog` 使用 `skill://` 主资源 locator，而 `SkillAccess` 需要 trusted filesystem/file locator。这个用户 Skill 的 RunResources→Shell→Sandbox 转换缺口仍待生产集成，本次验收只记录，不修改生产代码。
