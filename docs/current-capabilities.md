...
- Shell：`run_shell`，要求审批，默认经 macOS Seatbelt 执行并记录有界输出、进程终态和 Workspace 变化；明显只读查询不依赖完整仓库扫描，写入/未知命令保留有界完整性检查，失败时 UI 展示 canonical `code`、`summary` 和 `stderr`，索引不完整时保留 reconciliation。
- 工具发现：`tool_search`。
