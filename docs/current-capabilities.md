...
- Git mutation 和 Fetch 在写入 `in_progress` 前执行无副作用 preflight。Session、active Run、execution root、branch、path 或 remote 校验失败不会污染 `operations` 表。Fetch 同样复用现有 operations table、request hash 和 replay 规则。Fetch 的 deferred prepare 在单个 SQLite transaction 内同时创建 `operations` reservation 和 `async_operations` lifecycle。失败会完整回滚。
