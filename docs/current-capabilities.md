...
- `includeLocalChanges = true` 要求 source `HEAD == baseCommit`。Runtime 使用 Dulwich diff、Index 和 object primitives 捕获并 transfer tracked modified/deleted、staged、unstaged、binary、symlink、mode 和 untracked state。Source Workspace 不执行 stash、reset、checkout、add 或其他写入。Local-change state conflict、source change 和 materialization failure 会 rollback Worktree，无法安全清理时进入 `cleanup_required`。
...
- Runtime 提供 durable Worktree Snapshot、safe cleanup 和 Restore。Snapshot 使用压缩的 Dulwich-rendered full/staged patch、Pydantic manifest、SHA-256 artifact checksum 和 `refs/eidos/worktree-snapshots/<snapshot_id>` hidden ref。完整 dirty state 使用 Pydantic `GitWorkingTreeState` 保存 binary、symlink、mode、deleted、untracked 和 staged/index 数据。Snapshot 不保存 ignored environment。Restore 会重新 materialize `.worktreeinclude`，并恢复原 Worktree id、原 Worktree root、Snapshot HEAD 和原始 `baseCommit`。
...
- `GitBackend` 把 Git mechanics 与 Eidos Worktree lifecycle 分开。默认 `DulwichGitBackend` 使用 Dulwich public APIs 完成 refs、branch create/switch、status、diff、worktree、reset、normal clean、patch apply 和 hidden snapshot anchors。`GitWorkingTreeState` 用 Dulwich Index/Object primitives 完成精确 dirty capture/apply。`GitCliFallback` 只包含 executable-filter-safe worktree add 和 destructive `clean -fdx`；它不实现第二套 Git semantics，也不作为 Dulwich failure retry path。
- Dulwich read/status/diff 使用空配置避免 external diff、textconv、clean/process/smudge filter、fsmonitor 和 hook。Normal handoff cleanup 使用 Dulwich `reset(..., "hard")` 与 `clean`。Snapshot/retention cleanup 的 reset 使用 Dulwich；Dulwich 没有 ignored clean 的公开参数时只使用一个 bounded `clean -fdx` fallback。
