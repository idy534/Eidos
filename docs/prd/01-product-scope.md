...
- 随应用发布的只读系统 Skill，以及位于统一 Eidos Home 技能根的用户 Skill；v0.3 安装/创建 helper 由用户在系统 Terminal 显式运行。
...
- 多 Agent 与并行工具执行。
- MCP/Plugin 市场、Runtime 内远程安装与更新、浏览器自动化、Agent 自动执行 Skill 创建/安装和长期记忆。
