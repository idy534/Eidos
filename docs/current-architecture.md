...
Local → Worktree 没有 associated Worktree 时复用 WorktreeManager 的 prepared creation、source snapshot 和 Dulwich structured working-state transfer。当前 HEAD、staged、unstaged、untracked、binary、symlink、mode 和 committed changes 都进入目标 Worktree。已有 associated Worktree 时只验证并复用原来的 Worktree；缺失或 deleted 返回 `WORKTREE_RESTORE_REQUIRED`，invalid 返回 `WORKTREE_RECOVERY_REQUIRED`，Runtime 不创建第二个 Worktree。
...
`WorktreeSnapshot` 的 SQLite metadata 与 artifact store 分离。Artifact store 的路径是 `<EIDOS_DATA_DIR>/worktree-snapshots/<snapshot_id>/`，目录内保存 `full.patch.gz`、`staged.patch.gz`、`working-state.json.gz` 和 `manifest.json`。Artifact store 复用 `GitWorkingTreePatch`、`capture_worktree_changes` 和 `apply_worktree_changes`，并保存 tracked modified/deleted、staged、unstaged、untracked、binary、symlink 和 mode state。Artifact 使用 gzip、SHA-256、temporary directory、`os.replace`、file fsync 和 directory fsync。Ignored environment 不进入 artifact。Restore 会从当前 source repository 重新 materialize `.worktreeinclude` 和 ignored rule override。
...
WorktreeManager 保留 Eidos 的 Project、Worktree、Session、Run、operationId、SQLite lifecycle、recovery、compensation 和 Sandbox 语义。Git mechanics 通过 typed `GitBackend` 提供。默认 backend 是 `DulwichGitBackend`。它使用 Dulwich 处理 discovery、refs、branch、status、diff、worktree add/list/remove/prune、switch、patch apply 和 hidden snapshot ref。`GitWorkingTreeState` 使用 Dulwich Index/Object primitives 保存完整 dirty state。只有 executable filter safety 和 `clean -fdx` 这两个经源码确认的 Dulwich 公开 API 缺口，才进入 `GitCliFallback`；Runtime 不对失败的 Dulwich operation 自动切换 native path。Manager 不知道 subprocess result、porcelain 或 fallback runner。
