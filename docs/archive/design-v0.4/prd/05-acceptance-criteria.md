...
范围说明：本文是目标态验收集合，不是当前实现验收清单。第一期、第二期和第三期历史验收分别见 [MVP Lite](../../phases/mvp-lite.md)、[第二期实施范围](../../phases/mvp-phase-2.md) 与 [第三期实施范围](../../phases/mvp-phase-3.md)。
...
| 编号 | 验收项 | 标准 |
|---|---|---|
| A040 | Segment | 20 Steps 或 30 分钟后执行一次无工具收尾并进入 stopped |
| A041 | 后续处理 | 用户需要补充信息时创建新 Run |
...
| 编号 | 验收项 | 标准 |
|---|---|---|
| A064 | 瞬时模型故障 | 首 delta 前重试耗尽后 failed；多个 Attempt 只计一个 Step |
| A065 | 模型协议错误 | 无效响应零工具执行；连续两次后无工具收尾并 stopped，合法响应清零计数 |
...
| A078 | 敏感重试 | 模型连续两次生成敏感 ToolCall 后无工具收尾并 stopped，且不增加协议错误计数 |
