...
- 本轮代码、测试代码和当前能力文档已经修订。协议契约检查、Renderer 类型检查、Desktop 门槛、构建、原生 Seatbelt 和 Electron smoke 已通过。Runtime 完整测试沿用前一阶段结果，本轮没有修改 Runtime。真实 Desktop 人工验收尚未进行。
- 文件链接和产物卡打开当前 Workspace 内容。历史 ToolCall 保留执行时的 Diff；预览不提供历史文件字节快照。文件变化可能使资源版本失效，当前 Files 不提供手动刷新入口。环境信息中的“输出内容”只反映已分页读取的工具记录，不代表完整目录或工具成功清单。Files 不再显示结果文件列表。Projectless 对话不展示文本修改卡。HTML 卡片标题来自当前文件的 `<title>`；没有标题或读取失败时显示未命名网页。
- 支持内置预览的产物卡同时提供内置预览和系统应用打开。DOCX 只提供受控的系统应用打开。
- 文本修改卡会区分已完成、部分失败和待核验记录。重复修改同一路径时，界面不会累加各次补丁行数；如果没有可靠的合并 Diff，卡片会显示行数未完整统计。当前没有可精确恢复本轮修改的用户级 Checkpoint 入口，所以“撤销”按钮只显示不可用原因。
...
- Browser 是用户可操作的预览面板，尚未向 Agent 注册浏览器自动化 Tool。网页面板当前不提供标注入口，也没有独立的后退和前进 IPC，所以这两个导航按钮保持不可用。地址栏使用有限的域名、本地地址和空白判断，无法确认是地址时会提交 Google 搜索。
