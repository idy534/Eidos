...
- 2026-09-13 的产物声明修订只修改生产代码和当前文档。用户要求先确认代码再写测试并集中验证，因此本次未修改测试，也未运行测试、类型检查、lint、构建、原生 Seatbelt 或 Electron smoke。以前的测试通过记录不能作为本次验收结果。真实 Desktop 人工验收尚未进行。
- `declare_outputs` 只声明当前 Run Workspace 内已有的普通文件，不支持外部路径、目录、远程 URL 或历史文件字节快照。工具只核验元数据，不检查文件内容或渲染质量，不证明文件由本轮生成。每批最多 20 项；任一文件失败会使整个批次失败。系统提示要求调用声明工具，但模型仍可能漏调；漏调时只有原始文件和工具记录，界面不会猜测交付。当前没有撤回声明或历史版本下载功能。
...
- 支持内置预览的产物卡同时提供内置预览和系统应用打开。文档、演示文稿和 Excel 文件已有专属产物分类，但 DOC/DOCX、PPT/PPTX、XLS/XLSX/XLSM 仍只提供受控的系统应用打开。CSV/TSV 使用文本预览，尚无电子表格网格、公式计算或幻灯片内置查看器。本次分类和图标修订未修改测试，也未运行类型检查、测试、构建或 Desktop 验收。
