...
- 本次产物代码和当前文档已修订，但用户要求先确认代码，再编写测试并集中验证。本次没有编写或修改测试，也没有运行测试、lint、构建、smoke 或 Desktop 人工验收。历史测试通过记录不覆盖本次修订。
- 文件卡打开当前 Workspace 内容，并明确显示当前文件说明。历史 ToolCall 保留执行时的原始 Diff；系统没有历史交付字节快照。卡片和 Files 提供刷新入口。环境信息只显示已加载的工具记录；回复引用卡还没有进入这个列表。Projectless 同样显示文本修改卡。HTML 无标题或读取失败时使用文件名。
- 通用文件卡支持 PPTX、XLSX 和未知扩展名的发现与打开入口，但不代表这些格式已经有内置预览。Office 文件目前使用系统应用。系统不会自动转换 Office，也没有引入新的转换器依赖。
- 文本卡显示各次补丁的累计增删，不提供本轮最终净差异。部分缺失时显示已知统计和缺失文件数，完全缺失时显示原因。系统没有可精确恢复本轮修改的用户级 Checkpoint 入口，撤销仍不可用。
- 回复链接只提供候选文件路径。可读文件不等于本轮生成文件，也不等于内容通过验收。系统尚不提供持久化的主交付/辅助素材分组、交付版本比较、跨环境产物复制或高级发布。MCP 原生 resource_link 类型尚未接入；当前只识别已有 structuredContent 中的文件变化字段。
