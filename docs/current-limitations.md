...
- Git Project 可以创建 Local 或 Worktree Execution Session，也可以在同一个 Session 中执行 Local ↔ Managed Worktree Handoff。Managed Worktree 默认是 detached HEAD。Runtime 已提供 structured status、file-scoped diff、stage、unstage、commit、fetch、fast-forward-only pull、push、merge 和 merge abort typed API。Desktop 只接通了这些 Git workflow contract，尚未提供完整操作 UI。Rebase、stash、Diff Review UI 重构、inline comments 和 Real Checkpoint 尚未实现。
