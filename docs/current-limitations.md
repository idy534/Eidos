...
- 候选摘要必须通过 SQLite 事实验证，才能原子写入 verified record 和权威摘要。验证失败时，Runtime 保留上一份 verified summary。原始历史不会被删除。
