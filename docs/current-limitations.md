...
- Non-Git Project 已经支持 Direct Workspace Session。Non-Git Project 不支持 Git status、Git diff、Managed Worktree 或 Git-based Fork。Direct Checkpoint Fork 共享真实 workspace，不提供 directory snapshot、copy-on-write 或 filesystem rewind。
- Git Project 当前仍默认使用 Managed Worktree。Git staging、commit、push、pull、merge 和 rebase UI 尚未实现。
...
- Managed Session create、Session delete 和 managed Checkpoint Fork 已有 v17 durable lifecycle intent、startup reconciliation 和 same-operation retry。Runtime 仍会拒绝 dirty Worktree delete，并保留无法证明安全的目录和 branch。Direct Session delete 不删除用户 workspace。Runtime 不提供 branch merge、rebase、force delete 或 Git staging、commit、push UI。
