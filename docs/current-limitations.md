...
- 支持内置预览的产物卡同时提供内置预览和系统应用打开。文档、演示文稿和 Excel 文件已有专属产物分类，但 DOC/DOCX、PPT/PPTX、XLS/XLSX/XLSM 仍只提供受控的系统应用打开。CSV/TSV 使用文本预览，尚无电子表格网格、公式计算或幻灯片内置查看器。对应的分类、图标和 Workspace Reader 测试已经覆盖这些边界。
...
- 本次设置重构只完成生产代码。用户要求在确认后才编写测试并进入验证阶段，因此本次尚未更新测试和协议 Fixture，也没有执行 lint、build、自动化测试或人工 UI 验收。
