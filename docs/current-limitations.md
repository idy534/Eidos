...
- Git Project 可以创建 Local 或 Worktree Execution Session。Managed Worktree 默认是 detached HEAD。Phase 3B 支持只复制 ignored local files 的 `.worktreeinclude`、ignored rule override 自动 materialization、显式的 local changes transfer 和 `Create Branch Here`；Git staging、commit、push、pull、merge 和 rebase UI 尚未实现。
...
- Worktree Session create、Session delete、managed Checkpoint Fork 和 Create Branch Here 已有 v20 durable lifecycle intent、startup reconciliation 和 same-operation retry。Create Branch Here 使用 attach 时冻结的 `expected_head`，不使用创建时的 `base_commit` 判断当前 branch HEAD。Runtime 仍会拒绝 dirty Worktree delete，并保留无法证明安全的目录和 legacy attached branch。User branch 会在 Session delete 时保留。Local Session delete 不删除用户 workspace。Runtime 不提供 Local ↔ Worktree Handoff、Permanent Worktree、branch merge、rebase、force delete 或 Git staging、commit、push、Pull Request UI。
